...
void fadeLeftOnePitch(vector<string> objects, Engine &engine){
    ...
    {
        auto& objRef = engine.c2d.gui.get(obj);
        objRef.transform.position.x += 0.001f;
        objRef.texture.color.x += 0.01f;
        objRef.texture.color.y += 0.01f;
        objRef.texture.color.z += 0.01f;
    }
};
...
void changeSelectedOption(vector<string> options, int option, Engine &engine){
    ...
    {
        auto& objRef = engine.c2d.gui.get(obj);
        objRef.texture.color = {1.f, 1.f, 1.f, 1.f};
    }
    auto& selected = engine.c2d.gui.get(options[option]);
    selected.texture.color = {0.3f, 0.3f, 1.f, 1.f};
};
